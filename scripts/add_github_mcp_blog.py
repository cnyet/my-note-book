...
# 数据库配置
DATABASE_URL = f"sqlite:///{project_root}/data/ai_life_assistant.db"

# 文章内容
TITLE = "OpenCode 连接 GitHub MCP 服务完全指南"

SUMMARY = """
本文详细记录了在 OpenCode 中配置 GitHub MCP 服务的完整过程，包括遇到的问题、解决方案以及与 Claude Desktop 配置方式的对比分析。通过 Docker 方式成功实现了 OpenCode 与 GitHub MCP 的集成。
"""

CONTENT = """# OpenCode 连接 GitHub MCP 服务完全指南

## 📋 背景

Model Context Protocol (MCP) 是一个标准协议，允许 AI 应用与外部工具和数据源进行交互。GitHub 提供了官方的 MCP Server，使 AI 助手能够直接操作 GitHub 仓库、管理 Issues、PR 等。
