...
class LifeSecretary:
    """AI-powered lifestyle management secretary."""

    def __init__(self, config: Dict, config_path: str = "config/config.ini"):
        """
        Initialize life secretary.
        ...
        self.user_profile = {
            ...
            'location': '上海',
            'health_goals': {
                'weight': 'maintain',  # lose, maintain, gain
                'fitness_level': 'moderate',  # beginner, moderate, advanced
                'target_exercise_per_week': 3,
                'sleep_target': 8,  # hours
                'water_target': 2000,  # ml
                'dietary_preferences': {
    ...
    def run(self, save_to_file: bool = True) -> str:
        """
        Run the life secretary workflow.

        Args:
            save_to_file: Whether to save the plan to file

        Returns:
            Generated life management plan in markdown format
        """
        try:
            print("🌱 Life Secretary - Analyzing your lifestyle needs...")
            ...
            logger.error(f"Error in life secretary: {e}")
    ...
    def _get_recent_life_logs(self) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error reading recent life logs: {e}")
...
"""
    ...
    def _save_plan(self, plan: str):
        """
        Save life plan to file.

        Args:
            plan: Life plan content
        """
        try:
            ...
            self.file_manager.save_daily_file(
                file_type='life',
                content=content,
                custom_filename='今日生活.md'
            )

        except Exception as e:
            logger.error(f"Failed to save life plan: {e}")

    def interactive_mode(self):
        """
        Run life secretary in interactive mode.

        Allows user to input current status and get customized advice.
        """
        print("\n" + "=" * 70)
        print("🌱 Life Secretary - Interactive Mode")
        print("=" * 70)
        ...
        # Sleep quality
        sleep_quality = input("\nHow was your sleep last night? (excellent/good/fair/poor): ").strip().lower()
        ...
        # Customize prompt based on status
        ...
        if current_status:
            ...
            if current_status.get('exercised'):
                custom_context += f"- 已运动：{'是' if current_status['exercised'] else '否'}\n"
    ...
    def get_health_summary(self) -> str:
        ...
        try:
            ...
            logs = []
            for d in dates:
                path = f"data/daily_logs/{d}/今日生活.md"
            ...
            for content in logs:
                ex = self._extract_exercise_info(content)
                if ex.get('completed'):
                    exercise_count += 1
                    total_exercise_minutes += int(ex.get('duration', 0) or 0)

                sl = self._extract_sleep_info(content)
                if sl.get('hours', 0):
                    sleep_hours.append(float(sl['hours']))

                meals = self._extract_meal_info(content)
                if meals:
                    meal_days_ok += 1

                water = self._extract_water_info(content)
                target = self.user_profile['health_goals']['water_target']
                if water and target and water >= target:
                    water_goal_days += 1

                notes.extend(self._extract_notes(content))

            # Metrics
            total_days = len(logs) if logs else 0
            exercise_target = self.user_profile['health_goals']['target_exercise_per_week']
            exercise_rate = (exercise_count / exercise_target * 100) if exercise_target else 0
            avg_sleep = (sum(sleep_hours) / len(sleep_hours)) if sleep_hours else 0
            meal_rate = (meal_days_ok / total_days * 100) if total_days else 0
            water_rate = (water_goal_days / total_days * 100) if total_days else 0

            # Simple quality score
            sleep_score = round(min(max((avg_sleep / self.user_profile['health_goals']['sleep_target']) * 5, 0), 5), 1) if self.user_profile['health_goals']['sleep_target'] else 0
...
### 运动情况
- 目标：每周{exercise_target}次
- 完成：{exercise_count}次
- 完成率：{exercise_rate:.0f}%
- 总时长：{total_exercise_minutes}分钟

### 睡眠质量
- 平均时长：{avg_sleep:.1f}小时
- 目标达成率：{(avg_sleep / self.user_profile['health_goals']['sleep_target'] * 100) if self.user_profile['health_goals']['sleep_target'] else 0:.0f}%
- 质量评分：{sleep_score}/5

### 饮食情况
- 按时就餐天数：{meal_days_ok}/{total_days}
- 按时就餐率：{meal_rate:.0f}%
- 饮水目标达成：{water_goal_days}/{total_days} 天（{water_rate:.0f}%）

## 🏆 成就亮点
- 本周完成运动：{exercise_count}次
- 保持平均睡眠：{avg_sleep:.1f}小时
...
## 📋 下周目标
- 运动目标：每周{exercise_target}次，尽量平均分布
- 饮水目标：每日达到 {self.user_profile['health_goals']['water_target']}ml
"""
            if notes:
...
if __name__ == "__main__":
    ...
    # Create and run life secretary
    secretary = LifeSecretary(config_dict)
